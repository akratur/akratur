...
async function main() {
    ...
    for (const s of schools) {
        const { assignedTours, ...schoolData } = s;
        ...
        // Assign tours
        for (const assignment of assignedTours) {
            // Check if exists
            const existing = await prisma.schoolTour.findUnique({
                where: {
                    schoolId_tourId: {
                        schoolId: s.id,
    ...
    const surveys = [
        {
            ...
            options: JSON.stringify(["Kapadokya", "Çanakkale", "Pamukkale", "Safranbolu"]),
